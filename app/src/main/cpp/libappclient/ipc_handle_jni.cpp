...
static jobject getJLogEntryRecordObject(JNIEnv *env, const LogEntryRecord &log) {
    jclass clazz = env->FindClass("cc/ioctl/opdisplaytuner/ipc/daemon/IDispTunerDaemon$LogEntryRecord");
...
jobject createHalServiceStatusObject(JNIEnv *env, const IDispTunerDaemon::DaemonStatus::HalServiceStatus &status) {
    jclass clazz = env->FindClass("cc/ioctl/opdisplaytuner/ipc/daemon/IDispTunerDaemon$DaemonStatus$HalServiceStatus");
...
jobject createDaemonStatusObject(JNIEnv *env, const IDispTunerDaemon::DaemonStatus &status) {
    jclass clazz = env->FindClass("cc/ioctl/opdisplaytuner/ipc/daemon/IDispTunerDaemon$DaemonStatus");
    jmethodID constructor = env->GetMethodID(clazz, "<init>",
                                             "(ILjava/lang/String;ILjava/lang/String;Lcc/ioctl/opdisplaytuner/ipc/daemon/IDispTunerDaemon$DaemonStatus$HalServiceStatus;)V");
    ...
    jobject jopdispfeat = createHalServiceStatusObject(env, status.opDispFeatureHidlServiceStatus);
    jobject jstatus = env->NewObject(clazz, constructor, status.processId, jexecPath, status.abiArch,
                                     jcontext, jopdispfeat);
    return jstatus;
}
...
Java_cc_ioctl_opdisplaytuner_ipc_daemon_IpcNativeHandler_ntInitForSocketDir
    ...
    jclass cUtils = env->FindClass("cc/ioctl/opdisplaytuner/util/Utils");
...
Java_cc_ioctl_opdisplaytuner_ipc_daemon_IpcNativeHandler_ntPeekConnection
...
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_IpcNativeHandler_ntRequestConnection
        (JNIEnv *env, jclass) {
    IpcConnector &connector = IpcConnector::getInstance();
...
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_IpcNativeHandler_ntWaitForConnection
...
/*
 * Class:     cc_ioctl_opdisplaytuner_ipc_daemon_internal_NciHostDaemonProxy
 * Method:    isConnected
 * Signature: ()Z
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_isConnected
        (JNIEnv *, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    return connector.isConnected();
}
...
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_getVersionName
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
        env->ThrowNew(env->FindClass("java/lang/IllegalStateException"),
...
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_getVersionCode
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jstring JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_getBuildUuid
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jint JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_getSelfPid
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT void JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_exitProcess
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_isDeviceSupported
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_NciHostDaemonProxy_isHwServiceConnected
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_initHwServiceConnection
    ...
    if (env->GetArrayLength(jstrSoPathArray) == 0) {
...
//    env->ReleaseStringUTFChars(jstr2, strSoPath2);
    ...
    if (proxy == nullptr) {
        ...
        if (auto lpcResult = proxy->attachHwHidlService({path1});
...
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_waitForEvent
        (JNIEnv *env, jobject) {
    ...
    if (!g_RemoteDeathVec.empty()) {
        ...
        jmethodID ctor = env->GetMethodID(
                env->FindClass("cc/ioctl/opdisplaytuner/ipc/daemon/IDispTunerDaemon$RemoteDeathPacket"),
                "<init>", "(I)V");
    ...
    if (!g_IoEventVec.empty()) {
        ...
        jmethodID ctor = env->GetMethodID(
                env->FindClass("cc/ioctl/opdisplaytuner/ipc/daemon/internal/DispTunerDaemonProxy$RawIoEventPacket"),
                "<init>", "([B[B)V");
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jstring JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_IpcNativeHandler_getKernelArchitecture
        (JNIEnv *env, jclass) {
    struct utsname uts = {};
    if (uname(&uts) != 0) {
        int err = errno;
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jint JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_IpcNativeHandler_getIpcPidFileFD
        (JNIEnv *, jclass) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jobject JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_ntGetHistoryIoEvents
    ...
    if (proxy == nullptr) {
        ...
        if (auto lpcResult = proxy->getHistoryIoOperations(uint32_t(reqStartIndex), uint32_t(reqCount));
                !jniThrowLpcResultErrorOrException(env, lpcResult)) {
            IDispTunerDaemon::HistoryIoOperationEventList r;
            if (lpcResult.getResult(&r)) {
                uint32_t totalStart = r.totalStartIndex;
                uint32_t totalCount = r.totalCount;
                jclass clRawEventList = env->FindClass(
                        "cc/ioctl/opdisplaytuner/ipc/daemon/internal/DispTunerDaemonProxy$RawHistoryIoEventList");
                jclass clRawEvent = env->FindClass(
                        "cc/ioctl/opdisplaytuner/ipc/daemon/internal/DispTunerDaemonProxy$RawIoEventPacket");
                jmethodID ctorEvent = env->GetMethodID(clRawEvent, "<init>", "([B[B)V");
                jmethodID ctorList = env->GetMethodID(clRawEventList, "<init>",
                                                      "(II[Lcc/ioctl/opdisplaytuner/ipc/daemon/internal/DispTunerDaemonProxy$RawIoEventPacket;)V");
...
*/
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_clearHistoryIoEvents
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jobject JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_getDaemonStatus
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
        ...
        if (auto lpcResult = proxy->getDaemonStatus();
                !jniThrowLpcResultErrorOrException(env, lpcResult)) {
            IDispTunerDaemon::DaemonStatus r;
            if (lpcResult.getResult(&r)) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jint JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_deviceDriverIoctl0
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_isDisplayFeatureHidlServivceAttached
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_isDimLayerHbmEnabled
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
        ...
        if (auto lpcResult = proxy->isOpDimLayerHbmEnabled();
                !jniThrowLpcResultErrorOrException(env, lpcResult)) {
            bool r;
            if (lpcResult.getResult(&r)) {
                return r;
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT void JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_setDimLayerHbmEnabled
        (JNIEnv *env, jobject, jboolean jenable) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
        env->ThrowNew(env->FindClass("java/lang/IllegalStateException"),
        ...
        if (auto lpcResult = proxy->setOpDimLayerHbmEnabled(jenable);
                !jniThrowLpcResultErrorOrException(env, lpcResult)) {
            if (!lpcResult.isTransacted()) {
                env->ThrowNew(env->FindClass("java/lang/RuntimeException"),
                              "error while read data from LpcResult");
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_isDimLayerHbmForceEnabled
        (JNIEnv *env, jobject) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
        ...
        if (auto lpcResult = proxy->isOpDimLayerHbmForceEnabled();
                !jniThrowLpcResultErrorOrException(env, lpcResult)) {
            bool r;
            if (lpcResult.getResult(&r)) {
                return r;
 ...
 */
extern "C" [[maybe_unused]] JNIEXPORT void JNICALL
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_setDimLayerHbmForceEnabled
        (JNIEnv *env, jobject, jboolean jenable) {
    IpcConnector &connector = IpcConnector::getInstance();
    IDispTunerDaemon *proxy = connector.getNciDaemon();
    if (proxy == nullptr) {
        ...
        if (auto lpcResult = proxy->setOpDimLayerHbmForceEnabled(jenable);
                !jniThrowLpcResultErrorOrException(env, lpcResult)) {
            if (!lpcResult.isTransacted()) {
                env->ThrowNew(env->FindClass("java/lang/RuntimeException"),
                              "error while read data from LpcResult");
...
Java_cc_ioctl_opdisplaytuner_ipc_daemon_internal_DispTunerDaemonProxy_getLogsPartial
        ...
        if (auto lpcResult = proxy->getLogsPartial(uint32_t(start), uint32_t(count));
            ...
            if (lpcResult.getResult(&r)) {
                jclass clazzArray = env->FindClass(
                        "cc/ioctl/opdisplaytuner/ipc/daemon/IDispTunerDaemon$LogEntryRecord");
