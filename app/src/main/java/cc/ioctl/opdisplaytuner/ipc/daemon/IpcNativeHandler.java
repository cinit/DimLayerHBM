...
public class IpcNativeHandler {
    ...
    static DispTunerDaemonProxy mProxy = null;
    ...
    static class RemoteEventHandler implements Runnable {
        ...
        public void run() {
            try {
                while (mProxy != null) {
                    DispTunerDaemonProxy.NativeEventPacket packet = mProxy.waitForEvent();
                    if (packet != null) {
    ...
    public static void init(@NonNull Context ctx) {
        ...
        mProxy = new DispTunerDaemonProxy();
        synchronized (IpcNativeHandler.class) {
            if (!sInit) {
                System.loadLibrary("appclient");
    ...
    public static IDispTunerDaemon peekConnection() {
        checkProcess();
        if (ntPeekConnection()) {
            return mProxy;
        }
    ...
    public static IDispTunerDaemon connect(int timeout) {
        if (!sInit) {
    ...
    private static File extractDaemonExecutable() {
        ...
        File appLib = new File(ctx.getApplicationInfo().nativeLibraryDir, "libdisptunerd.so");
        if (appLib.exists()) {
            // check architecture
            try {
                int elfArch = NativeUtils.getElfArch(appLib.getAbsoluteFile());
                if (elfArch != NativeUtils.archStringToInt(kernelArch)) {
                    Log.e(TAG, "libdisptunerd.so is not for arch " + kernelArch + ", got " + elfArch);
                }
                // check executable bit
                if (appLib.canExecute()) {
                    return appLib;
                } else {
                    Log.e(TAG, "libdisptunerd.so is not executable");
                }
            } catch (IOException e) {
                Log.e(TAG, "get app lib ELF arch: IOException: " + e.getMessage());
            }
        }
        // we don't have the right libdisptunerd.so, so try to extract it
        String abiName = NativeUtils.archIntToAndroidLibArch(NativeUtils.archStringToInt(kernelArch));
        return NativeInterface.extractNativeLibFromApk("libdisptunerd.so", abiName, "disptunerd", true);
    }
     ...
     */
    public static IDispTunerDaemon startDaemonWithRootShell() throws RootShell.NoRootShellException, IOException {
        if (!sInit) {
            throw new IllegalStateException("attempt to connect before init");
        }
        initForSocketDir();
        // if the daemon is already running, return it
        IDispTunerDaemon daemon = connect(300);
        if (daemon != null) {
            return daemon;
        }
        ...
        // start the daemon with root shell, exec("disptunerd", uid, pid, fd)
    ...
    public interface IpcConnectionListener {
        void onConnect(IDispTunerDaemon daemon);

        void onDisconnect(IDispTunerDaemon daemon);
    }
